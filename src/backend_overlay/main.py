...
from backend_overlay.app.config import config
from backend_overlay.app.server import _start
from backend_overlay.shared.log import setup_logging
from backend_overlay.storage import get_storage_methods
...
def main():
    ...
    # at the path specified by backend_overlay.shared.log.get_log_file_path().
    # We probably want
...
def parse_settings():
    ...
    """ CLI Arguments """
    parser = argparse.ArgumentParser(description="Starts the Trust-me browser backend")
    parser.add_argument(
        "--testing",
        action="store_true",
        help="Run the Trust-me browser backend in testing mode using different ports and database",
    )
