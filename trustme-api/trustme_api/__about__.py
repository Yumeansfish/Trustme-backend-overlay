...
repo_root = projectpath.parent  # the browser backend repo root, in some circumstances
...
def detect_version_pkg() -> Optional[str]:
    try:
        return f"v{get_version('trustme-api')}.dev+{get_rev()}"
...
if __name__ == "__main__":
    # Use the browser backend repo root as working repo, if it exists
    if (repo_root / ".git").exists():
        print("Found browser backend repo root in parent dir, using for calls to git")
        workdir = repo_root
    else:
        workdir = projectpath
